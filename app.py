...
def load_investor_details(investors):
    ...
    # big investments
    big_invest_series = df[df['investor'].str.contains(investors)].groupby(
    ...
    vertical_series = df[df['investor'].str.contains(
        investors)].groupby('vertical')['amount'].sum()
    ...
    round_series = df[df['investor'].str.contains(
        'Sequoia Capital')].groupby('round')['amount'].sum()
    ...
    city_series = df[df['investor'].str.contains(investors)].groupby('city')[
        'amount'].sum()
...
def load_startup_details(startups):
    ...
    st.header(startups)
    st.subheader("Industry:  {}".format(
        df[df['startup'] == startups]['vertical'].iloc[0]))
    st.subheader("Sub Industry:  {}".format(
        df[df['startup'] == startups]['subvertical'].iloc[0]))
    st.subheader("Location:  {}".format(
        df[df['startup'] == startups]['city'].iloc[0]))
...
def load_overall_analysis():
    ...
    # max amount invested
    with col2:
        maxx = df.groupby('startup')['amount'].max().sort_values(
            ascending=False).head(1).iloc[0]
    ...
    if selected_option == 'Total':
        ...
        temp_df = df.groupby(['year', 'month'])[
            'startup'].count().reset_index()
        temp_df['x_axis'] = temp_df['month'].astype(
            'str')+'-'+temp_df['year'].astype('str')
